...
搜 HTTP 请求库调用且 URL 不是常量：`RestTemplate`、`WebClient`、`OkHttp`、`requests`、`axios`、`fetch`、`http.Get`、`reqwest`。记 URL 构造方式。

排除：固定 URL 的健康检查、监控上报。
...
[HTTP] POST /api/upload -> src/main/java/com/acme/FileController.java:56 uploadFile()
```

## 不遗漏检查

输出前确认：
- [ ] 6 个分类**都检查过**（即使某类没有发现也过一遍）
- [ ] 框架已知的所有入口注册模式都搜索了
- [ ] 内部调用都已排除
- [ ] 每个条目都有具体文件:行号
- [ ] 不确定的标记了 `[?]`
