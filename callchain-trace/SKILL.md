...
trace(func, depth=0, path=[], visited=set()):
  if func in visited → 检测到环，记叶子(状态=闭环)
    return
  if depth > 15 → 超出深度，记叶子(状态=超深)
    return
  if func 是外部库/无源码 → 边界，记叶子(状态=边界)
    return
...
|---|---|---|
| 无进一步核心调用 | 最底层 ✅ | 自然结束 |
| 仅非核心调用 | 跳过 ⛔ | 如 `log.info()` / `getTime()` |
| 检测到环 | 闭环 🔄 | 标注循环引用关系 |
| 外部库 / 无源码 | 边界 🔗 | 标注库名 + 调用签名 |
| 接口多实现 | 多态 ⚡ | 标注找到几个实现 |
| 深度 > 15 | 超深 ⚠ | 标注已追踪层级 |
...
├── {func_b}（跳过 ⛔ — 日志记录）
└── {func_c}（边界 🔗 — com.example.sdk:sendOrder）
    └── ...（继续展开）
...
| 函数 | 所在树 | 状态 | 说明 |
|---|---|---|---|
| {func_name} | 树1 | 最底层 ✅ | 无进一步核心调用 |
| {func_name} | 树1 | 跳过 ⛔ | 纯日志 `log.info(msg)` |
| {func_name} | 树2 | 边界 🔗 | 三方库 `SdkClient.send()` |
| {func_name} | 树2 | 多态 ⚡ | `Service.process()` 找到 2 个实现 |
| {func_name} | 树1 | 闭环 🔄 | `A → B → C → A` |
| {func_name} | 树1 | 超深 ⚠ | depth=16 时截断 |
```
