...
**断点续跑**：每个阶段通过检查产物目录中文件是否存在来判断哪些条目已完成，自动跳过已完成的条目。只有 Stage 0（collect）用 `.collect_done` 标记文件作整阶段跳过。
...
**Stage 0 — collect**（单次，检查 `.collect_done` 标记）：
```
if [ -f .vuln_agent_output/.collect_done ]; then
  跳过（已完成）
else
...
**断点续跑**：重复运行会自动跳过已完成条目（通过 glob 检查产物存在性），只处理未完成的部分。Stage 0 额外检查 `.collect_done` 标记文件。中途中断后直接重跑即可。
