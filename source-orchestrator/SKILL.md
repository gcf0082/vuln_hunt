...
对每个 stem 同时派发（一次 LLM 响应中发 ≤5 个 task）：
  subagent: source-vuln-planner
  prompt: 调用 vuln-planner skill
          - work_dir: .
          - task: {用户原始任务描述}
          - surface_file: {stem 相对路径，如 REST/iface-REST-user-list-0608-021435.md}
          产物: vuln_plans/{stem 相对路径}/
```

**Stage 3**（每 analyzed surface + vuln_plan 一个，5 并发）：
```
递归读 analyzed_surfaces/ 下所有 .md 得到 stem 列表
对每个 stem 同时派发：
  ...
  prompt: 调用 source-analyze-vuln skill
          ...
          - vuln_plans: vuln_plans/{stem 相对路径}/（可选，有则读取）
          产物: vuln_findings/{子目录/}{stem}-{n}.md
```

**Stage 4**（每 finding 一个，5 并发）：
```
