...
description: "Subagent 用技能：对一批文件逐一进行安全风险分析并定级。只要被分配分析文件的安全风险（审文件、文件级审计、风险定级），无论主流程是 source 分析还是 sink 分析，都要使用本技能。检测命令执行、代码执行、路径拼接文件操作、SQL 拼接、URL 拼接的外部变量注入，以及证书绕过、硬编码凭据、ReDoS、敏感信息日志。"
