...
1. **理解收集指令**：从用户输入里提取
   - **目标**（可选）：用户指定的具体业务入口，如"修改密码"、"订单导出"、"用户登录"。有目标时精确匹配；无目标时全量收集
   - **范围**：要扫哪个目录/项目/服务（路径、模块名）
   - **特征**：找什么类型的暴露面（注解、文件后缀、目录、配置项）。用户指定了 category（如 REST）时只收集该分类
...
4. **执行收集**：
   a. **精确匹配**（有明确目标时）：根据业务关键词（如"密码"/"changePwd"）搜索 URL 路由、控制器方法名、注解，定位匹配的接口。**只生成匹配的暴露面文件，不扫描无关接口**。只读源码直接可见的部分，不做跨文件追踪、不做动态分析、不挖调用链。**匹配不到时报告"未找到目标接口"，不得降级为全量扫描**。**用户指定了 category（如 REST）时只收集该分类，不收集其他分类**
   b. **全量扫描**（无明确目标时）：按特征全面扫描所有候选对象，不做关键词过滤。只读源码直接可见的部分，不做跨文件追踪、不做动态分析、不挖调用链
...
- **不重复**：同一个暴露面只生成一个文件
- **精确收集只出精确产物**：有明确目标时只写匹配的接口，非目标接口一律不生成文件。匹配不到就如实汇报，不得补扫无关接口充数
- **按需分类收集**：用户指定了 category（REST/MQ/SCRIPT/...）时只收集该分类，不收集其他无关分类
