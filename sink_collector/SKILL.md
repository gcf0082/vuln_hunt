---
name: collect-sinks
description: 仅在用户显式指名调用 collect-sinks 时触发，不要因模糊意图主动触发。
---
...
用户显式调用 `collect-sinks` 但**未指定 sink 类型**时，**必须**先反问：
...
4. **落盘产物**：每个 sink 写一个 `sink_list/{type}-{slug}-{n}.md`
5. **汇报**：sink 总数 + 分类统计
...
**函数**：{函数名}
**危险操作**：{具体描述}
**建议排查**：{sink_vuln_analyzer 应关注的方向}
```
...
- **不动源**：不修改任何源文件、配置文件
- **不替调用方决策**：幂等策略由调用方负责
- **失败显式标注**：写进 `.vuln_agent_output/meta/error/collect-sinks.md`

## 不在本 skill 范围内

- sink 的漏洞分析 → 交给 `sink_vuln_analyzer`
