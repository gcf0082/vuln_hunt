...
```
.vuln_agent_output/discovered_surfaces/
```
...
- **类型**：iface（或 noniface）
- **分类**：REST（或 SCRIPT 等）
- **来源**：/src/main/java/com/acme/UserController.java:48 list()
- **描述**：
...
| 来源 | 是 | **优先指向源码**：`文件路径:行号 函数名`（如 `UserController.java:48 listUsers()`）。无行号时 `文件路径 函数名()`。**只有 SCRIPT/CRON 等本身就是源码或纯配置的场景**才退化为只写 `文件路径`。如果一个暴露面同时涉及配置和实现类，**优先写实现类**，配置信息可在分类补充字段里带 |
...
| 分类 | 补充字段 |
|---|---|
| REST | `**URL**`（基本）<br>`**入口**`（基本）<br>`**参数**`（可选）<br>`**接口定义**`（可选，spec 文件路径，如 `api/swagger.yaml`） |
| MQ | `**队列/Topic**`（基本）<br>`**消费者**`（基本）<br>`**接口定义**`（可选，队列配置位置，如 `application.yml:42`） |
| gRPC | `**服务**`（基本）<br>`**方法**`（基本）<br>`**Proto**`（可选） |
| WebSocket | `**端点**`（基本）<br>`**处理器**`（基本） |
| GraphQL | `**字段**`（基本）<br>`**类型**`（可选）<br>`**Resolver**`（可选）<br>`**Schema**`（可选，`.graphqls` 文件路径） |
| SCRIPT | `**脚本名**`（基本）<br>`**语言**`（可选）<br>`**入口**`（可选） |
| TOOL | `**工具名**`（基本）<br>`**语言**`（可选）<br>`**入口**`（可选） |
| CRON | `**表达式**`（基本）<br>`**命令**`（基本）<br>`**文件**`（可选） |
| CLI | `**命令名**`（基本）<br>`**入口**`（可选） |
| SDK | `**SDK 名**`（基本）<br>`**语言**`（可选）<br>`**入口**`（可选） |

补充字段写在通用字段之后，每个字段独占一行。示例值参考原文件（REST 入口的 `UserController.java:48 listUsers()`、CRON 的 `*/5 * * * *` 等）。
...
- 漏洞分析、风险评级、利用验证 → 用其他 skill
- 修改源代码、加防护、加固建议 → 不在本 skill 范围
- 用户没给收集指令时主动全库扫描 → 不在本 skill 范围（要先问用户）
