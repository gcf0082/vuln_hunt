...
语句明显不完整（被截断、缺少闭合引号、语法不完整），**直接标记为疑似敏感 MEDIUM 并写入 hits/，不进入 Step 3-4**。
...
这类变量是常见的"容器"型变量名，其内容因场景而异，无法预先判断是否包含敏感数据。
同一个变量名在不同代码中可能完全不同：`data` 可能只含一个 ID，也可能包含完整的用户信息。
因为不确定里面有什么，所以必须标记为 MEDIUM 交由人工判断。
...
**3b 仅使用下方表格中列出的 6 类关键词。3a 中的变量名关键词不参与 3b 判断，两个表互不通用。**
格式化字符串中的词语即使与 3a 中的变量名相同（如 "result"、"data"、"params"、"query"），也只视为描述性文案，不触发标记。

```
示例 — 格式串中的描述性文字不触发:
  log.info("request params: {}", orderId)
  ✓ orderId 是 LOW（标识符），格式串中的 "params" 是日志文案，不是敏感关键词
  log.info("the select result of model is {}", apiName)
  ✓ apiName 是 LOW（name 结尾），格式串中的 "result" 是日志文案
```
...
示例（`hits/sensitive-logs-001.txt`）:
```
1#  logger.info("user password: %s", password)
2#  LOG.debug("request body: %s", body)
```
