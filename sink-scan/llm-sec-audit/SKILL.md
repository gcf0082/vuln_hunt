...
```
澄清参数 → 列出待分析文件 → 用户确认 → 逐文件调用 llm_prompt_cli → 保存结果 → 汇报
```
...
### 3. 逐文件调用 llm_prompt_cli

对每个文件执行：

```bash
llm_prompt_cli --input-file <代码文件绝对路径> --prompt-file <提示词模板路径> > <输出文件路径>
```

或使用内联提示词：

```bash
llm_prompt_cli --input-file <代码文件绝对路径> --prompt "<提示词内容>" > <输出文件路径>
```
