...
本 skill **只做一件事**：从用户消息中**识别意图** → 派发给 `source-orchestrator`、`sink-orchestrator`、`full-orchestrator` agent 或直接调子 agent。
...
| 分析当前目录的安全风险 | sink + source | 先 sink 从危险函数反向追踪已知风险点，再 source 从入口正向覆盖完整暴露面，最后合并汇总 |
...
| 帮我找出所有执行命令的代码 | 只收集（sink） | 直接扫 Runtime.exec/ProcessBuilder 等危险函数，列出位置，不做分析 |
| 帮我找出所有涉及文件操作的代码 | 只收集（sink） | 直接扫 File I/O 相关操作，列出位置，不做分析 |
| 找出所有疑似密码操作的代码 | 只收集（sink） | 直接扫密码相关操作，列出位置，不做分析 |

## 步骤

1. 读用户消息
2. 识别意图，提取 `vuln_type`（漏洞类型：命令注入→cmd、SQL注入→sql、路径穿越→path_traversal 等）。透传给下游 orchestrator
3. 扫暴露面 / 找入口 → 调 `source-orchestrator` agent，透传用户原消息
4. 查危险点 / 找 sink → 调 `sink-orchestrator` agent，透传用户原消息
5. 两种意图都看得出 → 调 `full-orchestrator` agent，透传用户原消息
6. 全面安全评估 / 安全风险 / 安全检查 → 调 `full-orchestrator` agent，透传用户原消息
7. 只收集 / 只采集 → 调 `surface-collector` 或 `sink-collector` agent（根据用户说的是入口还是危险点），透传用户原消息
8. 先 source 后 sink → 调 `full-orchestrator --order source-first` agent，透传用户原消息
9. 完全识别不出 → 反问「扫暴露面（source）还是查危险点（sink）？」

## 原则

- **识别意图，不机械匹配**：从用户消息的语义判断是 source 还是 sink
- **不解读上下文**：不读项目、不查结构，只读用户消息文本
- **透传用户原消息**：原封不动转给被派发的 agent
- **不写任何文件**：本 skill 唯一动作是调别的 agent
